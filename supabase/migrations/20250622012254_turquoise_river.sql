/*
  ...
  1. Nuevas Tablas
    - `document_analyses` - Historial de análisis de documentos
    - `supported_file_types` - Metadatos de tipos de archivo soportados
  ...
  3. Vistas
    - `recent_document_analyses` - Análisis recientes
    - `file_type_capabilities` - Capacidades por tipo de archivo
...
CREATE TABLE IF NOT EXISTS document_analyses (
  ...
  user_id TEXT, -- Para asociar con usuario si es necesario
...
-- 5. Crear vista para análisis recientes
...
-- 6. Función para obtener estadísticas de análisis
...
-- 7. Función para limpiar análisis antiguos
...
-- 8. Crear tabla para metadatos de tipos de archivo soportados
...
-- 9. Insertar tipos de archivo soportados
...
-- 10. Función para validar tipo de archivo
...
-- 11. Vista para obtener capacidades por tipo de archivo
...
-- Políticas para document_analyses
CREATE POLICY "Users can read their own analyses" ON document_analyses
  FOR SELECT USING (true); -- Ajustar según lógica de autenticación

CREATE POLICY "Users can insert their own analyses" ON document_analyses
  FOR INSERT WITH CHECK (true); -- Ajustar según lógica de autenticación

-- Política para supported_file_types (solo lectura)
CREATE POLICY "Everyone can read supported file types" ON supported_file_types
  FOR SELECT USING (true);

-- 13. Crear función para obtener historial de usuario
...
-- 15. Verificación final e información del esquema (sin loop problemático)
DO $$
DECLARE
...
BEGIN
  ...
  RAISE NOTICE '📋 Tipos disponibles: pdf, docx, doc, xlsx, xls, csv, txt, json';
END $$;
